...
class InterFrameAttention(nn.Module):
    def __init__(self, dim, motion_dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        # self.motion_proj = Linear(num_heads*7*7, motion_dim, r=32)
        self.motion_proj = Linear(num_heads*9*9, motion_dim)
    ...
    def forward(self, x1, x2, cor, H, W, mask=None):
        ...
        motion = attn.transpose(1, 2).reshape(B, N, -1)
...
class MotionFormer(nn.Module):
                ...
                if i < self.conv_stages:
                    ...
                    if i == self.conv_stages:
                        ...
                        patch_embed = OverlapPatchEmbed(patch_size=3,
                                                        ...
                                                        embed_dim=embed_dims[i])
                        down_channel = ConvBlock(embed_dims[i]*2,embed_dims[i],1)
                        setattr(self, f"down_channel{i + 1}", down_channel)
    ...
    def recurrent_propagation(self, img, appearence_back):
        B = img.shape[0] 
        x = img
        motion_features = []
        appearence_features = []
        ...
        for i in range(self.num_stages):
            ...
            down_channel = getattr(self, f"down_channel{i + 1}", None)
            ...
            if i < self.conv_stages:
                ...
                x = block(x)
                xs.append(x)
                appearence_back_current = appearence_back[i]
                appearence = torch.cat([x, appearence_back_current], dim=0)
                # print(appearence.shape)
                appearence_features.append(appearence)
            else:
                if i == self.conv_stages:
                    appearence_back_current = appearence_back[i]
                    ...
                    x, H, W = patch_embed(x)
                    appearence_back_current = appearence_back[i]
                    appearence_back_current = down_channel(torch.cat((appearence_back_current, x[:B].reshape(B, H, W, -1).permute(0, 3, 1, 2)) ,dim=1))
                    x = x[B:]
                cor = self.get_cor((x.shape[0]*2, H, W), x.device)
                appearence_back_current = appearence_back_current.flatten(2).transpose(1, 2)
                
                x = torch.cat([appearence_back_current, x], dim=0)
