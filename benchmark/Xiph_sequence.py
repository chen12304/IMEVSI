...
for strCategory in ['resized', 'cropped']:
    ...
    for strFile in ['BoxingPractice', 'Crosswalk', 'DrivingPOV', 'FoodMarket', 'FoodMarket2', 'RitualDance', 'SquareAndTimelapse', 'Tango']: 
        ...
        for intFrame in range(2, 99, 2):
            ...
            npyEstimate = model.hr_inference_sequence(tenFirst, tenSecond, model.af_down, model.af, down_scale=down_scale, flow=model.flow, mask= model.mask).clamp(0.0, 1.0).cpu()
            npyEstimate = padder.unpad(npyEstimate)[0]
